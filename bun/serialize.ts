...
export async function* serializerAsync(
  ...
  if (node.nodeType === 11) {
    yield* serializerAsync(
      Array.from((node as DocumentFragment).children),
      maxAwait
    );
    return;
  }
  ...
  if (node.nodeType === 1) {
    ...
    if ("isReady" in el) {
      ...
      console.warn("setting up a timeout for", maxAwait);
      ...
      try {
        await Promise.race([el.isReady, deferred.promise]).finally(() =>
    ...
    const attributesArray = Array.from(attributes.entries()).map(
      ([k, v]) => `${k}="${v}"`
    );
    const attributesString =
      attributesArray.length > 0 ? ` ${attributesArray.join(" ")}` : "";
...
async function* serializeChildrenOfNode(
