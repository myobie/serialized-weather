...
import { LocationsListElement } from "./locations-list-element.ts";

const isBrowser = !self.Bun;
...
export class GetLocationElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (isBrowser) {
      const geo = this.querySelector("button[data-geo]") as
        | HTMLButtonElement
        | undefined;
  ...
  handleChange(e: Event) {
    ...
    if (target.tagName === "SELECT") {
      ...
      const unit: TemperatureUnit =
        el.value === "fahrenheit" ? el.value : defaultTemperatureUnit;
  ...
  handleInput(e: InputEvent) {
    ...
    if (target.tagName === "INPUT") {
      if (this.searchValue && this.searchValue.length > 2) {
        ...
        this.#inputTimeout = this.ownerDocument.defaultView!.setTimeout(() => {
          if (
            this.searchValue &&
            this.#lastKnownSearchValue !== this.searchValue
          ) {
  ...
  updateLocations(locs: Loc[]) {
    ...
    const list = this.querySelector("locations-list") as
