...
import { defaultTemperatureUnit, getLoc, getWeather, type Loc, type TemperatureUnit, type Weather } from './weather.ts'
...
export class GetWeatherElement extends HTMLElement {
  ...
  handleWeather(w: Weather) {
    ...
    const hoursMarkup = w.hourly.map(hour => {
    ...
    const daysMarkup = w.daily.map(day => {
