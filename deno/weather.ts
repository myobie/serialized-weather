#!/usr/bin/env -S deno run --allow-net ./weather.ts

/*
 * Usage:
 * $ ./weather.ts New York
 * $ ./weather.ts Frankfurt
 */

if (import.meta.main) {
  const args = Array.from(Deno.args)
  args.shift()
  args.length === 0 && args.push('Berlin')

  const loc = await getLoc(args.join(' '))
...
export async function getLoc(searchOrId: string | number): Promise<Loc | undefined> {
...
export async function getWeather(loc: Loc, unit: TemperatureUnit = 'celsius'): Promise<Weather> {
