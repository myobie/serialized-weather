...
import { getEnvContext } from './env-context-event.js';
import { GeoEvent } from './geo.js';
import { getParams, listenForParamsChange, paramsChange } from './params.js';
import { defaultTemperatureUnit, getLoc, getWeather, type Loc, type TemperatureUnit, type Weather } from './weather.js';
...
export class GetWeatherElement extends HTMLElement {
	...
	async #handleCFGeo() {
		console.warn('possibly handling the cfGeo...');

		const params = getParams(this);

		if ('id' in params || 'search' in params) {
			console.warn('id or search, so returning early');
			return;
		}

		const view = this.ownerDocument.defaultView;
		// @ts-ignore
		const cfGeo = view.cfGeo as { latitude?: string; longitude?: string; nearbyName?: string } | undefined;

		if (cfGeo) {
			if (cfGeo.longitude && cfGeo.latitude) {
				const name = cfGeo.nearbyName || 'Nearby';
				this.dispatchEvent(new GeoEvent(parseFloat(cfGeo.latitude), parseFloat(cfGeo.longitude), name));
			}
		}
	}
	...
	handleGeo(e: GeoEvent) {
		console.warn('handling GeoEvent!');
	...
	handleParamsChange() {
		...
		if ('id' in currentParams && (this.#lastLoc?.id !== currentParams.id || this.#lastUnit !== unit)) {
	...
	getWeather(locOrId: Loc | number, tempUnit: TemperatureUnit = defaultTemperatureUnit) {
		this.#requestTag += 1;
	...
	async #getWeather(locOrId: Loc | number, tempUnit: TemperatureUnit, tag: number) {
		...
		try {
			...
			if (loc) {
				this.#lastLoc = loc;
				const env = getEnvContext(this);
				const weather = await getWeather(loc, tempUnit, env);
	...
	handleWeather(w: Weather) {
        ...
        <li>
          <span class=time-and-icon>
            ${icon && `<span class=icon><img alt="${icon}" src="/icons/wi-${icon}.svg" width=32></span>`}
            <span class=hour>${hour.time.getHours()}:00</span>
          </span>
          <span class=temp>${hour.temperature_2m}${w.hourly_units.temperature_2m}</span>
          <span class=feels-like>(${hour.apparent_temperature}${w.hourly_units.apparent_temperature})</span>
      ...
      `;
		...
		const daysMarkup = w.daily
			.map((day) => {
				...
				return `
        <div class=day data-weather-code="${day.weather_code}" data-icon="${icon}">
          <div class=leading>
            <h3>${title}</h3>
            ${icon && `<p class=icon><img alt="${icon}" src="/icons/wi-${icon}.svg" width=64></p>`}
          </div>
          <div class=trailing>
            <p class=temp>
              <span class=high>
                <abbr title=high>👆</abbr> ${day.temperature_2m_max}${w.daily_units.temperature_2m_max}
              </span>
              <span class=low>
                <abbr title=low>👇</abbr> ${day.temperature_2m_min}${w.daily_units.temperature_2m_min}
              </span>
            </p>
            <p class=precip>${day.precipitation_probability_max}% chance of precip</p>
            <p class=sun>
              <span>
      ...
      <div class=temps>
        <p>Temp: <strong>${w.current.temperature_2m}${w.current_units.temperature_2m}</strong></p>
        <p>Feels like: <strong>${w.current.apparent_temperature}${w.current_units.apparent_temperature}</strong></p>
      </div>
      ${icon && `<p class=icon><img alt="${icon}" src="/icons/wi-${icon}.svg" width=128></p>`}
      <h2>Hourly</h2>
