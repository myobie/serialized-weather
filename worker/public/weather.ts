...
function formatLocName(loc: Pick<Loc, 'country' | 'admin1' | 'name' | 'country_code'>) {
...
export async function getLocs(search: string, env?: Env): Promise<Loc[]> {
	const url = new URL('https://geocoding-api.open-meteo.com/v1/search');
	url.searchParams.set('name', search);
	const response = await apiRequest(url, env);
	...
	const json = (await response.json()) as { results: Omit<Loc, 'formatted_name'>[] } | undefined;
...
export async function getLoc(searchOrId: string | number, env?: Env): Promise<Loc | undefined> {
	...
	const json = (await response.json()) as Omit<Loc, 'formatted_name'> | undefined;
...
async function apiRequest(url: string | URL, env?: Env): Promise<Response> {
	if (typeof url === 'string') {
		url = new URL(url);
	}

	if (env?.API_KEY) {
		url.searchParams.set('apikey', env.API_KEY);

		// the api host for customers is different
		url.host = `customer-${url.host}`;
	}

	if (env?.RATE_LIMITER) {
		const outcome = await env.RATE_LIMITER.limit({ key: 'everywhere' });
...
export async function getWeather(loc: Loc, unit: TemperatureUnit, env?: Env): Promise<Weather> {
	...
	const weather: Weather = {
		...
		daily_units: { ...json.daily_units, time: 'Date', sunrise: 'Date', sunset: 'Date' },
		daily: json.daily.time.map((_, index) => {
			const day: Weather['daily'][number] = {
				...
				precipitation_probability_max: json.daily.precipitation_probability_max[index],
			};
			return day;
		}),
	};
