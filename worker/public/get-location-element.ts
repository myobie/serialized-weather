...
import { GeoEvent, getGeo } from './geo.js';
import { getParams, listenForParamsChange, paramsChange, pushParams, pushURL } from './params.js';
import { defaultTemperatureUnit, getLocs, type Loc, type TemperatureUnit } from './weather.js';
import { type LocationsListElement } from './locations-list-element.js';
...
export class GetLocationElement extends HTMLElement {
	...
	#template = (() => {
      ...
      <form>
        ...
        <button type=button data-geo disabled><abbr title="Current location">📍</abbr></button>
	...
	connectedCallback() {
		...
		const isBrowser = view && (!('isServer' in view) || !view.isServer);
	...
	handleChange(e: Event) {
		...
		if (target.tagName === 'SELECT') {
			...
			const el = target as unknown as HTMLSelectElement;
	...
	async #getLocations(search: string, tag: number) {
		...
		try {
			...
			const locs = await getLocs(search, env);
	...
	updateLocations(locs: Loc[]) {
		...
		const list = this.querySelector('locations-list') as unknown as LocationsListElement | undefined;
